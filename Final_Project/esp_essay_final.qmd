...
rbind(words_h10, words_t10) %>% 
  mutate(`Used More By` = ifelse(diff > 0, "Consultant Not Used", "Consultant Used")) %>%
...
ej_freq_df <- convert(ej_dfm_dict, 'data.frame') %>% 
  ...
  mutate(rec_jus = identifying_disadvantaged_communities,
         ...
         rec_jus_std = (rec_jus - mean(rec_jus, na.rm=TRUE))/ sd(rec_jus, na.rm=TRUE),
         proc_jus_std = (proc_jus - mean(proc_jus, na.rm=TRUE))/ sd(proc_jus, na.rm=TRUE),
         dis_jus_std = (dis_jus - mean(dis_jus, na.rm=TRUE))/ sd(dis_jus, na.rm=TRUE),
         mhi1000 = mhi / 1000,
         pop1000 = population / 1000,
         consultant_binary = as.factor(consultant_binary))

rec_mod <- glm(rec_jus ~  plan_year + pop1000 + per_latino + per_black + mhi1000 + plan_type + plan_length + consultant_binary,
     data = ej_freq_df,
     family = 'poisson')

proc_mod <- glm(proc_jus ~  plan_year + pop1000 + per_latino + per_black + mhi1000 + plan_type + plan_length + consultant_binary,
     data = ej_freq_df,
     family = 'poisson')

dis_mod <- glm(dis_jus ~  plan_year + pop1000 + per_latino + per_black + mhi1000 + plan_type + plan_length + consultant_binary,
     data = ej_freq_df,
     family = 'poisson')

# rec_mod <- lm(sqrt(rec_jus_std) ~  plan_year + pop1000 + per_latino + per_black + mhi1000 + plan_type + plan_length + consultant_binary,
#      data = ej_freq_df)
# 
# proc_mod <- lm(sqrt(proc_jus_std) ~  plan_year + pop1000 + per_latino + per_black + mhi1000 + plan_type + plan_length + consultant_binary,
#      data = ej_freq_df)
# 
# dis_mod <- lm(sqrt(dis_jus_std) ~  plan_year + pop1000 + per_latino + per_black + mhi1000 + plan_type + plan_length + consultant_binary,
#      data = ej_freq_df)
...
node_type <- ifelse(all_nodes %in% city_nodes, TRUE, FALSE)
