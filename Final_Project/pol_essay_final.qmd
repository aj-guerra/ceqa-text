...
mod_1 <- glm(cosine ~  any_shared_consultant + same_year + same_pop + same_mhi + both_eje + both_gp + both_he,
             family = quasibinomial(),
            data = ej_sim_df) 

mod_3 <- glm(cosine ~  same_year + same_pop + same_mhi + both_eje*any_shared_consultant + both_gp*any_shared_consultant + both_he*any_shared_consultant ,
             family = quasibinomial(),
            data = ej_sim_df) 
...
key_table <- data.frame(keys=character(), 
                        terms=character())
  
for (key in keys){
  terms <- ej_dictionary[[key]]
  terms_for_table <- paste(terms, collapse=', ')
  kt <- data.frame(keys=key,
                   terms=terms_for_table)
  key_table <- rbind(key_table, kt)
