...
freq <- textstat_frequency(plan_dfm, force=TRUE)
...
plan_sim_df <- tibble(as.data.frame(textstat_simil(plan_dfm,  method='cosine'))) %>% 
  ...
  mutate(same_year = as.factor(ifelse(plan_year.x == plan_year.y, 1, 0)),
         ...
         any_shared_consultant = as.factor(mapply(function(a, b) length(intersect(a, b)) > 0, consultant_list.x, consultant_list.y)),
         n_shared_consultants = mapply(function(a, b) length(intersect(a, b)), consultant_list.x, consultant_list.y),
...
# 
# mod_1 <- lm(log(cosine) ~ any_shared_consultant,
#             data = plan_sim_df) 

mod_2 <- lm(log(cosine) ~  any_shared_consultant + same_year + both_eje + both_gp + both_he,
            data = plan_sim_df) 

mod_3 <- lm(log(cosine) ~  same_year + both_eje*any_shared_consultant + both_gp*any_shared_consultant + both_he*any_shared_consultant ,
            data = plan_sim_df) 

mod_5 <- lm(log(cosine) ~  same_year + both_eje*any_shared_consultant + both_gp*any_shared_consultant + both_he*any_shared_consultant + same_pop + same_mhi,
            data = plan_sim_df) 
